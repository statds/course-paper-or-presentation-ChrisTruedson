...
\section*{Keywords}
AdaBoost; Bagging; Credit Risk; Decision Trees; Feedforward Neural Networks; Gradient Boosting; K-Nearest Neighbors (KNN); Loan Default Prediction; Machine Learning; Random Forests; Recurrent Neural Networks (RNN); XGBoost

\section{Introduction}
\label{sec:intro}

Approximately 22.7 million Americans have personal loans, 45 million have student loans, and 51.5 million households have mortgages with the vast majority of all these loans coming from banks and similar financial institutions. As such, it is crucial that these businesses are able to efficiently, accurately, and with the greatest level of optimization determine who it is safe to loan money to and who is likely to default. This not only matters for the financial stability and profitability of these institutions but also has significant implications for the broader economy and the financial well-being of individuals.

Many studies have addressed the issue of using machine learning to predict whether a customer will default on a loan or not using various different methods like decision trees and random forests \citet{madaan2021loan}, KNN approaches \citet{lai2020loan}, and deep neural nets \citet{bayraci2019deep} to name a few. However, these approaches often face challenges in terms of balancing accuracy, precision, and recall, particularly in predicting loan defaults accurately. This highlights a gap in the current literature and methodologies, where existing methods may not sufficiently address the complexities inherent in loan default prediction.

This paper contributes to the field by conducting a comprehensive analysis of several machine learning models, including some not extensively explored in existing literature for this purpose, such as advanced ensemble techniques and different neural network architectures. This paper aims to identify models that not only provide high overall accuracy but also maintain a balanced precision and recall, especially for predicting default cases.
